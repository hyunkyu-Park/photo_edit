...
def add_file():
    files = filedialog.askopenfilenames(title = "이미지 파일을 선택하세요", \
        ...
        initialdir = r"C:\Users\park6\PycharmProjects\MySecondGame\images") #최초에 사용자가 지정한 경로를 보여줌
...
def start():
    ...
    #파일 목록 확인
    if list_file.size() == 0:
        msgbox.showwarning("이미지 파일을 추가하세요")
        return
...
# #1.가로 넓이 옵션
# #가로 넓이 레이블
lbl_width = Label(option_frame, text = "가로 넓이", width = 8)
